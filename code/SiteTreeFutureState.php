...
class SiteTreeFutureState extends DataObjectDecorator
{
     ...
     */
    public static function set_future_datetime($datetime)
    {
        if ($datetime) {
     ...
     */
    public static function get_future_datetime()
    {
     ...
     * session.
     */
    public static function choose_future_datetime()
    {
        $date = false;
        
        if (empty($_GET['stage']) && empty($_GET['archiveDate']) && isset($_GET['futureDate'])) {
            if ($time = strtotime($_GET['futureDate'])) {
     ...
     */
    public function modelascontrollerInit()
    {
