...
class SiteTreeCMSWorkflow extends DataObjectDecorator
{
     ...
     * @return boolean
     */
    public function setEmbargo($date, $time = null)
    {
        if (!$time) {
     ...
     * @return boolean
     */
    public function setExpiry($date, $time = null)
    {
        if (!$time) {
            // split $date by space to get two time components, which happens on form submission
            $parts = explode(" ", $date);
        ...
        if ($wf = $this->openWorkflowRequest()) {
            if ($wf->ExpiryField()) {
                ...
                $timestamp = strtotime("$year-$month-$day $time");
                if ($timestamp) {
                    if ($wf->EmbargoDate && $timestamp < strtotime($wf->EmbargoDate)) {
                        return false;
                    }
     ...
     */
    public function canChangeEmbargo()
    {
     ...
     * Can we change the embargo date - only if there is an open workflow request
     */
    public function canChangeExpiry()
    {
    ...
    {
        if (self::$allowed_request_classes) {
            foreach (self::$allowed_request_classes as $class) {
    ...
    public function openOrNewWorkflowRequest($workflowClass, $notify = true)
    {
        if ($wf = $this->openWorkflowRequest($workflowClass)) {
            return $wf;
        } elseif (is_subclass_of($workflowClass, 'WorkflowRequest')) {
    ...
    {
        if ($wf = $this->openWorkflowRequest()) {
            if (get_class($wf) == 'WorkflowPublicationRequest') {
                $wf->approve(_t('SiteTreeCMSWorkflow.AUTO_APPROVED', "(automatically approved)"));
            } else {
                $wf->deny(_t("SiteTreeCMSWorkflow.AUTO_DENIED_PUBLISHED", "(automatically denied when the page was published)"));
            }
        }
    ...
    {
        if ($wf = $this->openWorkflowRequest()) {
            if (get_class($wf) == 'WorkflowDeletionRequest') {
                $wf->approve(_t('SiteTreeCMSWorkflow.AUTO_APPROVED', "(automatically approved)"));
            } else {
                $wf->deny(_t("SiteTreeCMSWorkflow.AUTO_DENIED_DELETED", "(automatically denied when the page was deleted)"));
            }
        }
    }
