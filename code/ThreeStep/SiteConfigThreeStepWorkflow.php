...
class SiteConfigThreeStepWorkflow extends DataObjectDecorator
{
    ...
    public function canApprove($member = null)
    {
        ...
        // check for empty spec
        if (!$this->owner->CanApproveType || $this->owner->CanApproveType == 'Anyone') {
            return true;
        }
        
        // check for any logged-in users
        if ($this->owner->CanApproveType == 'LoggedInUsers' && !Permission::checkMember($member, 'CMS_ACCESS_CMSMain')) {
            return false;
        }

        // check for specific groups
        if ($this->owner->CanApproveType == 'OnlyTheseUsers' && (!$member || !$member->inGroups($this->owner->ApproverGroups()))) {
            return false;
        }
