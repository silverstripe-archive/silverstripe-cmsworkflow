...
class LeftAndMainCMSWorkflow extends LeftAndMainDecorator
{
    ...
    public function cms_setembargoexpiry($data)
    {
            ...
            if (!$wfRequest->CanChangeEmbargoExpiry()) {
                ...
                if (isset($data['ResetEmbargo'])) {
                    ...
                    if (isset($data['EmbargoDate'])) {
                        ...
                        if ($tz) {
                            $embargoField->setConfig('usertimezone', $tz);
                        }
                    ...
                    if (isset($data['ExpiryDate'])) {
                        ...
                        if ($tz) {
                            $expiryField->setConfig('usertimezone', $tz);
                        }
                    ...
                    } elseif (isset($data['ExpiryDate']) && !$expiryTimestamp) {
                        $error = "Expiry date/time is not valid";
                    } elseif (isset($data['EmbargoDate']) && $embargoTimestamp < time()) {
                        $error = "Embargo date/time must be AFTER the current server date/time";
                    } elseif (isset($data['ExpiryDate']) && $expiryTimestamp < time()) {
                        $error = "Expiry date/time must be AFTER the current server date/time";
                    } elseif ($embargoTimestamp && $expiryTimestamp && $embargoTimestamp > $expiryTimestamp) {
     ...
     */
    public function workflowAction($workflowClass,  $actionName, $id, $comment)
    {
        if (is_numeric($id)) {
