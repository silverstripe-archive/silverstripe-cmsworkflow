			...
			<% if ClassName == WorkflowPublicationRequest %>
				...
				<% if EmbargoField %>
					<p id="embargoExpiry-embargoStatus" style="display:<% if EmbargoDate %>block<% else %>none<% end_if %>">
						This page is currently scheduled to be published at <span id="embargoDate">$EmbargoDate.Full</span><% if ExpiryField.DefaultTimezoneName %>, $ExpiryField.DefaultTimezoneName time<% end_if %>.
				...
				<% if ExpiryField %>
					<p id="embargoExpiry-expiryStatus" style="display:<% if ExpiryDate %>block<% else %>none<% end_if %>">
							This page is currently scheduled to be unpublished at <span id="expiryDate">$ExpiryDate.Full</span><% if ExpiryField.DefaultTimezoneName %>, $ExpiryField.DefaultTimezoneName time<% end_if %>.
